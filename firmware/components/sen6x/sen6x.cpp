  ...
  namespace sen6x
  {
    ...
    this->set_timeout(stop_measurement_delay, [this]() {
      ...
      if (this->voc_sensor_ && this->store_baseline_) {
        ...
        // FIXED: Use standard C++ macros for build time
        std::string build_time = __DATE__ " " __TIME__;
        uint32_t hash = fnv1_hash(build_time + std::to_string(combined_serial));
      ...
      *run_step = [this, steps, run_step](size_t index)
      {
        ...
        steps[index]();
        this->set_timeout(20, [run_step, index]()
                          { (*run_step)(index + 1); });
      };
      ...
      *poll_ready = [this, poll_ready, poll_retries, read_cmd, read_words](uint8_t retries_left)
      {
        ...
        if ((raw_read_status & 0x0001) == 0)
        {
          ...
          this->set_timeout(50, [this, poll_ready, retries_left]()
                            { (*poll_ready)(retries_left - 1); });
          return;
        }
