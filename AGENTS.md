...
├── components/
│   ├── aether_display/
│   │   ├── aether_display.h             # E-paper driver, display modes, button handlers
│   │   ├── aether_display_layout.h      # Shared layout rendering (sketch layout, drawing primitives)
│   │   └── fonts/                       # GFX font headers (.h) and source TTFs
│   └── aether_ui/
│       ├── __init__.py                  # ESPHome codegen (CONFIG_SCHEMA + to_code)
│       ├── aether_ui.h                  # Web UI component (AetherUI class, API handlers)
│       ├── aether_ui_html.h             # Auto-generated: inlined HTML/CSS/JS (do not hand-edit)
│       └── web/
...
A YAML `interval` block calls `aether::aether_display::tick_and_draw(...)` every **500ms**, passing all nine sensor values plus the temperature unit flag (`!id(temp_unit_c_switch).state` — true = Fahrenheit).

`aether_display.h` manages four display modes:
- **MODE_BOOT** — animated "Syntropy // Labs" wordmark with slash animation; transitions to normal when all metrics are valid.
- **MODE_NORMAL** — main air quality dashboard rendered by `aether_display_layout::render_sketch_layout()`.
...
`aether_ui` is a custom ESPHome component that registers an `AsyncWebHandler` on port 80. It serves:
...
`__init__.py` runs `generate_aether_ui_html()` at codegen time. This reads `web/index.html`, `web/style.css`, and `web/app.js`, inlines CSS/JS into the HTML, and writes the result to `aether_ui_html.h` as a C++ raw string literal (`INDEX_HTML`). **Do not hand-edit `aether_ui_html.h`** — edit the source files in `web/` instead.
...
- **Short press** (50ms–1s): calls `aether_display::on_short_press()` — toggles between MODE_NORMAL and MODE_INFO
- **Long press** (≥3s): calls `aether_display::on_long_press()`:
...
| YAML sensor ID | `aether_ui:` key | `__init__.py` conf key | C++ setter          | Display lambda arg |
|---------------|-------------------|----------------------|--------------------|--------------------|
...
Fonts are pre-converted Adafruit GFX font headers in `firmware/components/aether_display/fonts/`. Each `.h` file is declared in `esphome.includes` in `aether.yaml` and referenced via `extern const GFXfont` in `aether_display_layout.h`.
...
When adding a new font: create the `.h` header with `#pragma once`, add it to `esphome.includes` in `aether.yaml`, and `#include` it in `aether_display.h`.

## Display Layout Details (aether_display_layout.h)
...
2. `firmware/aether.yaml` — add the ID mapping under `aether_ui:`
3. `firmware/aether.yaml` — add `id(new_sensor).state` to the `interval` lambda's `tick_and_draw()` call
4. `firmware/components/aether_ui/__init__.py` — add `CONF_*` constant, add to `CONFIG_SCHEMA`, add to the `to_code` wiring loop
5. `firmware/components/aether_ui/aether_ui.h` — add `set_*()` setter, private `Sensor*` field, include in `handle_state_()` JSON output
6. `firmware/components/aether_display/aether_display.h` — add cached `g_*` variable, update `tick_and_draw()` signature, update `all_metrics_ready()`
7. `firmware/components/aether_display/aether_display_layout.h` — add field to `Metrics` struct, add rendering in `render_sketch_layout()`
8. `firmware/components/aether_ui/web/app.js` — add to the `items` array in `renderMetrics()`
...
2. `firmware/aether.yaml` — add the switch ID to `aether_ui:` mapping if the web UI needs access
3. `firmware/components/aether_ui/__init__.py` — add config key, schema entry, and `to_code` wiring
4. `firmware/components/aether_ui/aether_ui.h` — add setter, field, API handler if needed
...
- `aether_display` uses free functions in a nested namespace (not a class)
- `aether_ui` uses the `AetherUI` class extending `esphome::Component` and `AsyncWebHandler`
- `aether_display_layout` uses templated free functions for display-type independence
...
- `CONFIG_SCHEMA` and `to_code` must stay strictly aligned with the C++ `AetherUI` API
- For each config field: schema validation entry + variable fetch in `to_code` + matching C++ setter call
- `generate_aether_ui_html()` runs at import time before codegen to inline web sources into `aether_ui_html.h`

## Web UI Conventions

- Source files live in `web/` (HTML, CSS, JS); they are inlined into `aether_ui_html.h` at compile time
...
- Do not hand-edit `aether_ui_html.h`
